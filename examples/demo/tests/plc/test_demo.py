...
from demo.plc.control import PLCConnection, FaultInjector, measurement_fault
...
from demo.plc.control import load_bindings
...
def shared_runtime(tmp_path_factory):
    runtime = DemoRuntime(artifacts=tmp_path_factory.mktemp("demo-runs"), hold=0)
    runtime.start()
    yield runtime
    runtime.stop()
...
def test_all_five_real_pytest_cases_repeat_and_restore(runtime, repeat):
    owned_process = runtime.process.process
    owned_endpoint = runtime.endpoint
    ...
    log = (Path(run["artifacts"]) / "pytest.log").read_text(encoding="utf-8")
    ...
    events = [json.loads(line) for line in (Path(run["artifacts"]) / "events.jsonl").read_text(encoding="utf-8").splitlines()]
    for case in CASES:
        ...
        if case["id"] != "normal":
            kinds = [event["kind"] for event in own]
            assert kinds.index("injection_trigger") < kinds.index("injected")
    assert client.get("/api/artifacts/junit.xml").status_code == 200
    assert_baseline(runtime)
    assert runtime.process.process is owned_process
    assert runtime.endpoint == owned_endpoint
    assert owned_process.poll() is None


def test_cancel_armed_injector_before_pumping(runtime):
    injector = FaultInjector(runtime.endpoint, contract=runtime.contract)
    try:
        injector.arm("timeout")
        assert not injector.first_write.is_set()
    finally:
        injector.stop()
    assert not injector.thread.is_alive()
    runtime.manual("pump")
    until(lambda: runtime.manual_controller.snapshot()["status"] != "running")
    runtime.manual_controller.stop()
    assert runtime.manual_controller.snapshot()["status"] == "completed"
    assert not injector.first_write.is_set()
...
def fail_reset(monkeypatch):
    ...
    monkeypatch.setattr(PLCClient, "reset", reset)
...
'''
    ...
    manifest = write_connection(tmp_path, runtime.endpoint, runtime.contract)
    events_file = tmp_path / "events.jsonl"
    environment = demo_environment()
    environment["PYTEST_DISABLE_PLUGIN_AUTOLOAD"] = "1"
    result = subprocess.run([sys.executable, "-m", "pytest", "-c", str(Path(__file__).resolve().parents[2] / "pytest.ini"), "-p", "demo.plc.pytest_plugin",
                             "--rootdir", str(tmp_path), "--confcutdir", str(tmp_path),
                             "--demo-connection", str(manifest), "--demo-events", str(events_file),
                             "--demo-cancel-file", str(tmp_path / "cancel"), "--demo-hold", "0", "-q", str(test_file)],
                            cwd=str(tmp_path), env=environment, stdin=subprocess.DEVNULL, capture_output=True, text=True, timeout=30,
                            **hidden_process_options())
    assert events_file.exists(), result.stdout + result.stderr
    ...
    if stage != "teardown":
        reset_index = messages.index("写入任务已退出，诊断与 Reset 已完成")
