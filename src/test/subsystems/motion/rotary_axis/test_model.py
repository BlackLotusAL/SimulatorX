...
from subsystems.motion.rotary_axis import model
from subsystems.motion.rotary_axis.model import Result, RotaryAxis
...
def test_enable_home_and_move_are_distinct_operations(monkeypatch):
    monkeypatch.setattr(model, "PARAMETERS", replace(model.PARAMETERS, initial=30))
    axis = RotaryAxis()
...
def test_reset_removes_motion_faults_and_reference(monkeypatch):
    monkeypatch.setattr(model, "PARAMETERS", replace(model.PARAMETERS, initial=5))
    axis = ready_axis()
    axis.execute("MoveAbsolute", {"angle": 100})
    axis.advance(0.3)
    axis.set_fault("stalled", True)
    ...
    axis.reset()
    assert axis.snapshot() == RotaryAxis().snapshot()
...
def test_invalid_device_constants_rejected(settings):
    with pytest.raises(ValueError):
        replace(model.PARAMETERS, **settings)
