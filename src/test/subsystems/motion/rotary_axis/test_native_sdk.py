...
from subsystems.motion.rotary_axis.model import CONTROL_ERROR
from subsystems.motion.rotary_axis import build
...
@pytest.fixture
def native_sut(device, native_driver, request):
    sdk_axis = device.subsystems["motion"].hardware["rotary_axis"].client
...
def test_real_shared_library_transfers_parameters_outputs_and_consumes_overrides(device, native_sut):
    sdk_axis = device.subsystems["motion"].hardware["rotary_axis"].client
    sdk_returns = device.subsystems["motion"].hardware["rotary_axis"].client.returns
    call = native_sut
...
def test_native_limit_and_stall_feedback(device, native_sut):
    sdk_axis = device.subsystems["motion"].hardware["rotary_axis"].client
    call = native_sut
    call("enable")
    call("home")
