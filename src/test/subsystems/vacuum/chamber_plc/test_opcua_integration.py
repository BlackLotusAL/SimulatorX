...
from protocols.opcua.client import reset_nodes
from protocols.opcua.bindings import read_values
...
def test_all_eight_nodes_are_native_writable_scalars(device):
    ...
    for key, node in plc_nodes.items():
        ...
        binding = plc_client.bindings[key]
        node.set_value(binding.baseline, binding.ua_type)
    assert reset_nodes(plc_client, plc_client.definition) is True


def test_repeated_and_combined_native_writes_need_no_intermediate_reset(device):
    plc_nodes = device.subsystems["vacuum"].hardware["chamber_plc"].nodes
...
def test_wrong_wire_type_is_rejected_without_changing_node(device, key, variant):
    plc_nodes = device.subsystems["vacuum"].hardware["chamber_plc"].nodes
...
def test_remote_quality_timestamps_and_reset_reuse_same_connection(device):
    plc_client = device.subsystems["vacuum"].hardware["chamber_plc"].client
    plc_nodes = device.subsystems["vacuum"].hardware["chamber_plc"].nodes
    pressure = plc_nodes["vacuum.pressure_pa"]
    ...
    reset_nodes(plc_client, plc_client.definition)
    for key, data in read_values(plc_client, plc_nodes).items():
        assert data.Value.Value == plc_client.bindings[key].baseline
...
def test_bad_numeric_pressure_does_not_stop_server(device, value):
    plc_client = device.subsystems["vacuum"].hardware["chamber_plc"].client
    plc_nodes = device.subsystems["vacuum"].hardware["chamber_plc"].nodes
...
def test_native_pressure_write_is_used_on_the_next_tick(device):
    plc_nodes = device.subsystems["vacuum"].hardware["chamber_plc"].nodes
    plc_nodes["vacuum.command"].set_value(1, ua.VariantType.UInt16)
...
def test_reset_preserves_subscription_and_connection(device):
    plc_client = device.subsystems["vacuum"].hardware["chamber_plc"].client
    plc_nodes = device.subsystems["vacuum"].hardware["chamber_plc"].nodes
...
def test_create_process_stops_and_releases_port():
    process = opcua_process().start()
...
def test_port_conflict_leaves_existing_process_alive():
    with opcua_process() as existing:
        from urllib.parse import urlparse
        duplicate = opcua_process(urlparse(existing.info["opcua_endpoint"]).port)
        try:
